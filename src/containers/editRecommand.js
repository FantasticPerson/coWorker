...
export default class CreateRecommand extends Component{
    ...
    onConfirm=()=>{
        ...
        const {updateRemand,hideEdit,data} = this.props
        let contactText = `${this.contentArr.indexOf(contact)}-{${contactValue}}`
        let typesText = chipArr.join(',')
        updateRemand(titleValue,contentValue,contactText,typesText,data.id)
        hideEdit()
    }

    componentDidMount(){
        const {data} = this.props
        let arr = data.types.split(',')
        for(let i = 0;i<arr.length;i++){
            arr[i] = parseInt(arr[i])
        }
        let contactIndex = parseInt(data.contact[0])
        this.setState({chipArr:arr,contact:this.contentArr[contactIndex]})
    }
            ...
            <Dialog open={true} maxWidth={false} onClose={this.onCancel}>
                ...
                <DialogContent>
                    <div style={{width:'800px'}}>
                        ...
                        <TextField  inputRef={(input)=>{this.titleRef = input}} defaultValue={data.title} fullWidth style={{marginTop:'20px'}} label="请输入标题" placeholder="标题"></TextField>
                        <TextField inputRef={(input)=>{this.contentRef=input}} defaultValue={data.content} fullWidth style={{marginTop:'20px'}} label="请输入内容" placeholder="内容" multiline rows="4"/>
                        <TextField
                            select
                        ...
                        <TextField inputRef={(input)=>{this.contactRef = input}} defaultValue={data.contact.substr(3,data.contact.length-4)} style={{marginTop:'20px',width:'300px'}} label="输入内容" placeholder="内容"/>
                    </div>
                </DialogContent>
                <DialogActions>
